...
//****NOTA**** -> En el archivo pasado todas las cuisine American tienen un espacio al final, por eso he puesto esta consulta así
db.restaurant.find( { "cuisine": { $ne: "American " }, "grades.score": { $gt: 70 }, "address.coord.0": { $lt: -65.754168 } })


//12 Escriu una consulta per trobar els restaurants que no preparen menjar 'American' i tenen algun score superior a 70 i que, a més,
//és localitzen en longituds inferiors a -65.754168.
...
//tenen alguna nota 'A' i no pertanyen a Brooklyn. S'ha de mostrar el document segons la cuisine en ordre descendent
...
//restaurants que contenen 'ces' en les últimes tres lletres en el seu nom
...
//17 Escriu una consulta per trobar els restaurants que pertanyen al Bronx i preparen
//plats americans o xinesos
db.restaurant.find( { $and: [{"borough": "Bronx"}, { $or: [{"cuisine": "American "},{"cuisine": "Chinese"}]}]})


//18 Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per aquells
//restaurants que pertanyen a Staten Island, Queens, Bronx o Brooklyn
...
//aquells restaurants que NO pertanyen a Staten Island, Queens, Bronx o Brooklyn
...
//21 Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells
//restaurants que preparen marisc ('seafood') excepte si son 'American', 'Chinese' o el name del restaurant comença amb lletres 'Wil'
db.restaurant.find( { $and: [{ "cuisine": "Seafood",  "borough": { $nin: [ "American ", "Chinese", "Bronx", "Brooklyn" ]  }}]}, {"restaurant_id": 1, "borough": 1,"cuisine": 1} )

//22 Escriu una consulta per trobar el restaurant_id, name i grades per a aquells restaurants que aconsegueixin
//un grade de "A" i un score de 11 amb un ISODate "2014-08-11T00:00:00Z"
db.restaurant.find( { $and: [  { "grades.grade": "A"}, { "grades.score": 11 }, { "grades.date": ISODate("2014-08-11T00:00:00Z") } ]}, {"restaurant_id": 1, "name": 1, "grades.grade": 1} )

//23 Escriu una consulta per trobar el restaurant_id, name i grades per a aquells restaurants
//on el 2n element de l'array de graus conté un grade de "A" i un score 9 amb un ISODate "2014-08-11T00:00:00Z"
db.restaurant.find( { $and: [  { "grades.1.grade": "A"}, { "grades.score": 9 }, { "grades.date": ISODate("2014-08-11T00:00:00Z") } ]}, {"restaurant_id": 1, "name": 1, "grades.grade": 1} )

//24 Escriu una consulta per trobar el restaurant_id, name, adreça i ubicació geogràfica per a
//aquells restaurants on el segon element del array coord conté un valor entre 42 i 52
...
//30 Escriu una consulta que seleccioni el restaurant_id, name i grade per a aquells restaurants
...
//31 Escriu una consulta per trobar el name de restaurant, borough, longitud, latitud i cuisine
//per a aquells restaurants que contenen 'mon' en algun lloc del seu name
db.restaurant.find( { "name": /mon/ } ,{"name": 1, "borough": 1, "address.coord.0": 1, "address.coord.1": 1, "cuisine": 1} )

//32 Escriu una consulta per trobar el name de restaurant, borough, longitud, latitud i cuisine 
//per a aquells restaurants que conteinen 'Mad' com a primeres tres lletres del seu name
db.restaurant.find( { "name": /^Mad/ } ,{"name": 1, "borough": 1, "address.coord.0": 1, "address.coord.1": 1, "cuisine": 1} )
